...
final class ICloudChannelHandler: NSObject {
  ...
  /// 把备份文件夹交给系统「文件」选择器，用户可存进看得到的 iCloud 云盘。
  private func exportToDrive(result: @escaping FlutterResult) {
    DispatchQueue.main.async {
      do {
        let root = try self.backupRoot()
        let files = try FileManager.default.contentsOfDirectory(
          at: root,
          includingPropertiesForKeys: nil,
          options: [.skipsHiddenFiles]
        )
        let urls = files.isEmpty ? [root] : files
        let picker = UIDocumentPickerViewController(forExporting: urls, asCopy: true)
        guard let presenter = Self.topViewController() else {
          result(
            FlutterError(code: "icloud", message: "无法打开文件选择器", details: nil)
          )
          return
        }
