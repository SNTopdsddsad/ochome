...
- Migration: `from >= 3 && from < 10` runs `createTable(worlds)` (IF NOT EXISTS) then `addColumn(roles, roles.worldId)` only when `PRAGMA table_info(role)` lacks `world_id`. The guard exists because v9 was used by two parallel branches (worlds / relationships) and v10 unifies them. The `from < 3` rebuild branch creates `worlds` **before** `roles` (the latest role definition references it) and must skip the `addColumn`. Follow the same two-branch shape for any future world column.
...
| Restore ≤ v8 snapshot | Opens, migrates to v10, `world` empty, `role.world_id` null |
| Restore v9 snapshot from the relationships branch | Opens, migrates to v10, `world` created, `role.world_id` added as null |
